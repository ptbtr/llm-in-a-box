...
async function main(): Promise<cdk.App> {
  ...
  const imageName = unwrapOrRaise(t.string.decode(dc.services['worker'].image),
    `image name not found in docker-compose.yml`).split(":")[0];
  const tag = unwrapOrRaise(t.string.decode(dc.services['worker'].build?.target),
